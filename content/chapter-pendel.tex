...
\subsubsection{Füllen des \lstinline{Data} Objektes}
...
Diese Gleichung nutzt die Varianz der einzelnen Datensammlungen um damit die entsprechende Steigung zu verwenden.
Unter berücksichtigung das diese bereits Implementationen haben, welche in Listing~\ref{lst:data_variance} nachgesehen werden können, muss hierfür also nurnoch die Kovarianz $\sigma_{xy}^2$ berechnet werden.
Diese kann entsprechend in der \lstinline{DataXY} Klasse definiert werden, welche für die Tests die entsprechenden Punkte vorhalten kann.
