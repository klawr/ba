...
Als Pendel wird mittels \name{mec2} ein einfaches Modell definiert, welches dann von verschiedenen Höhen aus losgelassen wird.
...
% TODO Das kann sich noch ändern... stimmt das noch?
% TODO Verteilung des X und Y Wertes betrachten. Diese könnten eine Standardabweichung erzeugen.
...
Hierfür wurde die \code{simulate} Function, welche in Listing~\ref{lst:pendel} gezeigt wurde mit dem Programmcode aus Listing~\ref{lst:simulate_compareImages} versehen.
...
Wobei \code{ctx1} und \code{ctx2} den \code{RenderingContext} des jeweiligen HTML-Canvas Elementes repräsentieren.
\code{compareImages} wird in Listing~\ref{lst:compareImages} gezeigt.
...
\begin{figure}[htb]
	\includegraphics[width=\textwidth]{gfx/compareImages_1.png}
