...
Bei einer Parallelkurbel bei, welcher sich die Koppel nicht bewegt, würde der Momentanpol ebenfalls stets im Unendlichen liegen.
...
Diese Formel soll nun angewendet werden, um den Momentanpol von Elementen zu bestimmen dessen Bewegung in Videosequenzen nachvollzogen wird.
...
Andererseits würde dieses nicht schlupffrei rollen können\footnote{Bei einem rutschendem Rad würde sich der Momentanpol entsprechend im Mittelpunkt des Rades befinden.}.
...
Da kein \name{mec2} Modell definiert ist werden die entsprechenden Befehle, wie in Kapitel~\ref{ch:simulation_js} beschrieben, ignoriert.
...
Im Gegenteil lässt hier ein besonders ungenaues Ergebnis den Schluss zu, dass es sich entsprechend um ein sich translativ bewegendes Glied handelt.
Um zu verhindern, dass der Momentanpol fälschlicherweise auf der Gliedebene des sich bewegten Elementes befindet wird der eingeschlossene Winkel durch \lstinline{dw = dw > Math.PI / 2 ? dw - Math.PI : dw;} korrigiert wie in Listing~\ref{lst:group_momentanpol} gesehen werden kann, damit jeweils den kleineren der beiden eingeschlossenen Winkel bevorzugt.
