...
% TODO Erwähne, dass dieses Vektorfeld analog zum Gössner S.119 beschriebenen Vektorfeld steht.
...
Einen Vorschlag einer nutzbaren Implementation wurde von Lucas und Kanade bereits 1981 vorgeschlagen. % TODO cite
...
Beschreibt man das erste Bild für die entsprechenden Pixelwerte als eine Funktion $F(x)$ und die Pixelwerte des zweiten Bildes entsprechend als $G(x)$, so wird der Vektor gesucht, welcher den Fehler zwischen diesen beiden Bildern minimiert.
...
% TODO erkläre Shi-Tomasi hier ein bisschen, so muss später nurnoch die reine Implementation beschrieben werden.
...
Die Funktionen zur Bestimmung interessanter Punkte und dessen Verfolgung wird von \name{opencv.js} bezogen. % TODO cite
\name{opencv}\footnote{\name{opencv} ist eine Abkürzung für Open Computer Vision.} ist eine Sammlung optimierter Funktionen im Bereich der Bildverarbeitung und des Maschniellen sehens.
Implementiert ist \name{opencv} in \name{C++}.
...
Als Parameter übergeben werden hier ein Bild (in Graustufen), eine \lstinline{cv.Mat} Objekt in welchem die Ergebnisse gespeichert werden, die maximale Anzahl der Punkte welche zur Verfolgung gesucht werden, dessen Qualität, % TODO wie Qualität?
der minimale euklidische Abstand der Punkte zueinander. % none und blocksize? Hä?

Die darauf folgende Berechnung von \lstinline{cv.calcOpticalFlowPyrLK} gibt dann nach dem Vergleich zweier Bilder die prognostizierte neue Position der durch \lstinline{goodFeaturesToTrack} bestimmten Punkte.
...
Abschließend wird dem Algorithmus ein \lstinline{criteria} Parameter übergeben. % TODO den man nochmal angucken sollte was genau der nochmal macht...
